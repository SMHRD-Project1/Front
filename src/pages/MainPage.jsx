...
const MainPage =  forwardRef((props, ref) => {
  ...
  useEffect(() => {
    ...
    console.log(markerRef.current[0])
  ...
  useEffect(() => {
    if (polymap_) {
      result = markerList.map(item =>
        `${item.x}%20${item.y}`).join(',');
  ...
  useEffect(() => {
    fetch('/data/seogu.geojson')
      ...
      .then(data => {
        // console.log("행정동 GeoJSON 데이터:", data.features);
        setDong(data.features)
        // console.log("행정동 dong 데이터:", dong);
      })
      .catch(error => {
  ...
  const dongEvent = () => {
    // console.log("in");
    dong.forEach((item) => {
      if (item.properties.adm_nm === "양동") {
        dongPositionsRef.current = item.geometry.coordinates[0]
        // console.log(dongPositionsRef.current);
        clearAll();
